...
function toast(t, x = "") {
  let e = $("#toast");
  if(!e) return;
  const tt=$("#toastTitle"), tx=$("#toastText"); if(tt)tt.textContent=t;if(tx)tx.textContent=x;
...
const modules = {
  ...
  calendar: () =>
    '<div class="section-bar"><div><h2>Calendar · September 22</h2><small class="muted">3 barbers · 82% utilization</small></div><div class="filters"><button class="active">Day</button><button>Week</button><button>List</button><button data-action="add-block">+ Block</button></div></div><div class="calendar-board">' +
    calendar() +
...
modules.live = () =>
  ...
  "</p></article></div>" +
  (state.liveSlotFilled
    ? '<section class="resolved-card"><span>✓ SLOT FILLED</span><h2>Demo Waitlist Guest accepted the 14:30 appointment.</h2><p>Calendar, customer history, forecast, Impact, Decision Log and Business Memory were updated.</p><button class="primary" data-go="decisions">View decision trail</button></section>'
